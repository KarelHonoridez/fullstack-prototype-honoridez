...
function handleRegister() {
    ...
    window.db.accounts.push({
        ...
        role: 'user', verified: false, createdAt: new Date().toISOString()
    });
...
function handleLogin() {
    ...
    if (!account) {
        setInvalid(emailEl);
        showFieldError(errorBox, 'No account found with that email.');
        return;
    }
    if (account.password !== password) {
        setInvalid(passwordEl);
        showFieldError(errorBox, 'Incorrect password.');
        return;
    }
    if (!account.verified) {
        localStorage.setItem('unverified_email', email);
